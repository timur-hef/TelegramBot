...
from horoscope import date, signs
...
def weather(message, session=None):
    try:
        ...
        logger.info(
            f"User {message.from_user.username} id-{message.from_user.id} using bot (weather)"
        )

        if user and getattr(user, "location", None):
            text = (
                "Ты уже вводил местоположение. Хочешь использовать предыдущее? Тогда отправь точку '.'\n"
...
def stonks(message):
    try:
        logger.info(
            f"User {message.from_user.username} id-{message.from_user.id} using bot (stonks)"
        )
...
def horoscope(message, session=None, change=False):
    try:
        user = session.query(User).filter(User.id == message.from_user.id).first()
        logger.info(
            f"User {message.from_user.username} id-{message.from_user.id} using bot (horoscope)"
        )
        ...
        if user and getattr(user, "horo_sign", None) and not change:
            ...
            b9 = telebot.types.InlineKeyboardButton(
                "Стрелец", callback_data="sagittarius"
            )
            b10 = telebot.types.InlineKeyboardButton(
                "Козерог", callback_data="capricorn"
            )
            b11 = telebot.types.InlineKeyboardButton(
                "Водолей", callback_data="aquarius"
            )
            ...
            if change:
                ...
                bot.send_message(
                    chat_id=message.chat.id, text=text, reply_markup=buttons
                )
    except Exception as e:
        logger.error("Ошибка сервера:\n")
        logger.error(e)
...
def horoscope_timeline(
    callback_query: telebot.types.CallbackQuery = None, message=None, sign=None
):
    if callback_query:
    ...
    b6 = telebot.types.InlineKeyboardButton(
        "Сменить знак зодиака", callback_data="change"
    )
...
@bot.callback_query_handler(
    func=lambda c: (c.data.split()[0] in signs) and (c.data.split()[1] in date)
)
